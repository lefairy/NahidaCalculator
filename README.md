...
- 界面与存档调整(必读)：  
  1、基础属性中的其他属性和属性转化改为使用属性组的方式区分，可以给每一组属性自定义命名，方便管理来自多个角色、多套圣遗物、多个命座的增益。属性组带有开关功能，可以快捷启用或禁用一组属性
    以便对比和记录。  
  2、时间轴中的技能增加开关，功能类似上一条其他属性中的开关。  
  3、由于前两条调整，存档中的数据结构也有同步调整，新版本计算器读取旧版本存档时会将基础属性中的其他属性和属性转化放到默认属性组中，并将时间轴中的所有技能置于禁用状态，可以使用全选按钮
    重新将所有技能设置为启用。旧版本计算器无法正常读取新版本存档中的其他属性和技能。  
- 新增功能：  
  1、支持月结晶、星超导、星扩散反应，增加“反应层数”属性用于计算直伤星超导和反应星扩散的系数，增加“反应基础伤害提升%”属性以更方便准确地计算类似伊涅芙天赋提高14%月感电基础伤害的效果。  
  2、支持欢愉伤害计算，增加对应的笑点和欢愉度属性，注意实际游戏中欢愉技使用笑点增伤，其他技能造成的欢愉伤害使用好活当赏增伤。本计算器未提供增笑属性，可以使用治疗护盾乘区模拟。  
- 其他优化：  
  1、在选项页面增加了使用说明功能，目前包含了部分设置项的说明和部分计算公式说明。  
  2、切换计算模式时会在下拉列表中自动隐藏不相关属性（这些属性不会在不相关的模式中生效），如原神模式隐藏速度、笑点等，星铁模式隐藏元素精通、反应层数等。  
  3、调整了文字颜色以减少视疲劳。  
- 问题修复：  
  1、修复了属性转化中源属性范围的上限效果异常的问题。  

### v5.2更新内容：

- 新增功能：  
  1、支持月绽放、月感电反应。在本计算器中，剧变反应中的月感电为反应月感电，增幅反应中的月感电和月绽放为直伤月感电、直伤月绽放。  
  2、支持角色和敌人等级调整，调整角色等级时会同步调整防御系数和剧变反应、击破的基础伤害。由于原神未开放91\~94级和96\~99级，在计算时剧变反应基础伤害的91\~94级伤害同90级，96\~99级同95级。  
  3、增益类型增加独立乘区、伤害值提升、反应伤害值提升。各个独立乘区增益互相乘算，伤害值提升类似申鹤羽毛、昔时套，反应伤害值提升类似菈乌玛Q（不对蒸发融化反应生效），可选是否允许两种伤害值提升增益享受独立乘区的加成。  
- 功能优化：  
  1、存档能够保存伤害计算模式（原神、星穹铁道），读取旧版本存档不会修改计算模式。增加了对话框以提示当前读取的存档是否包含计算模式，可在选项中关闭对话框。  
  2、支持多起点词条分配，降低在计算词条分配时陷入局部最优的概率，可在设置中打开或关闭。由于计算量增加，步长递进优化功能默认开启，其开关已移除。  
- 问题修复：  
  1、修复了技能的选项中敌方防御和敌方抗性开关的功能异常的问题。此外，打开自身防御、抗性、减伤开关后不再计算敌方防御和抗性。  
  2、修复了读取存档后时间轴长度的滑动条中滑块位置未更新的问题。  
...
2、计算模式(原神/星铁)现在能够被存档保存，读档时自动切换，但5.2版本前(不包含5.2)的版本存档中未保存相关信息，读取时无法自动切换。无论什么模式下敌方易伤都是一个单独的乘区，不会与增伤加算(原神游戏中如莫娜的星异效果为使敌方受伤增加，但实际和增伤在同一个乘区，与增伤加算)  
