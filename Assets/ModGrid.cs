...
{
    [AddComponentMenu("Layout/ModGrid")]
    /// <summary>
    /// Layout class to arrange child elements in a grid format.
    /// </summary>
    /// <remarks>
    /// The ModGrid component is used to layout child layout elements in a uniform grid where all cells have the same size. The size and the spacing between cells is controlled by the ModGrid itself. The children have no influence on their sizes.
    /// </remarks>
    public class ModGrid : LayoutGroup
    {
        /// <summary>
        ...
        [SerializeField] protected bool m_AutoWidth = false;
        ...
        /// <summary>
        /// Which constraint to use for the ModGrid.
        /// </summary>
        /// <remarks>
        /// Specifying a constraint can make the ModGrid work better in conjunction with a [[ContentSizeFitter]] component. When ModGrid is used on a RectTransform with a manually specified size, there's no need to specify a constraint.
        /// </remarks>
        ...
        [SerializeField] protected bool m_AutoColumn = false;
        ...
        public override void CalculateLayoutInputHorizontal()
        {
            ...
            SetLayoutInputForAxis(
                padding.horizontal + spacing.x * minColumns - spacing.x,
                padding.horizontal + spacing.x * preferredColumns - spacing.x,
                -1, 0);

            if (needRefresh) active.Refresh(transform);
        }
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            if (axis == 0)
            {
                ...
                if (m_Constraint == Constraint.FixedColumnCount && m_AutoWidth)
                {
                    cellSize = new Vector2((width - padding.horizontal - m_Spacing.x * (m_ConstraintCount - 1)) / m_ConstraintCount, cellSize.y);
                }
                else
                {
